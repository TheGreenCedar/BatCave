...
The per-machine Windows installer creates no Public Desktop or all-users Start Menu shortcut. This avoids treating a user-deletable shared `.lnk` as trusted machine registration. Until the separate per-user launch-surface work lands, launch `BatCave Monitor\batcave-monitor.exe` from the machine's Program Files folder or create a user-owned pin/shortcut; user-created entries are not installer authority. See the [shared-shortcut retirement decision](docs/decisions/0013-windows-shared-shortcut-retirement.md).
