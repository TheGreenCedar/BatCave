...
The [public-artifact install smoke harness](public-artifact-install-smoke.md) consumes the process-local verifier receipt for planning and offers fixture-driven contract tests. The [native executor boundary](native-install-smoke-executor.md) owns an exact private copy through a process-local JavaScript capability, but its owned-byte verification receipt remains artifact-only. The [Linux adapter source boundary](linux-native-install-smoke-adapter.md) remains a contract-only deb/AppImage surface.

Published deb releases also run the [protected post-public Ubuntu smoke](linux-deb-post-public-smoke.md) on a fresh `ubuntu-22.04` GitHub-hosted runner. That job compares anonymous public bytes to the exact pre-publication candidate inventory, verifies checksums and source-bound attestations again, installs and purges the deb through owned transient systemd units, and exercises identity, settings, degradation, and advancing core telemetry as a standard user. Its sanitized result is deliberately a post-public observation with `release_evidence_eligible: false`; it does not promote the underlying current-user `native_candidate` packet into a #98 `release_evidence` packet.
