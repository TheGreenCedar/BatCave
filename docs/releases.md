...
The [public-artifact install smoke harness](public-artifact-install-smoke.md) consumes the process-local verifier receipt for planning and offers fixture-driven contract tests. The [native executor boundary](native-install-smoke-executor.md) owns an exact private copy through a process-local JavaScript capability, but its owned-byte verification receipt remains artifact-only. The [Rust complete-operation entry decision](decisions/0004-rust-install-smoke-complete-operation-entry.md) records why those process-local brands cannot be serialized into a Rust executor and why no production Rust root is registered yet. The [Linux adapter source boundary](linux-native-install-smoke-adapter.md) registers the closed deb/AppImage profiles and fixed process-settlement contract, but no platform command consumes package bytes. Source execution therefore returns `skipped` and cannot emit native or release proof. A future Rust-owned operation must independently reverify the public release and exact bytes before a reviewed adapter can consume them or emit release evidence.
