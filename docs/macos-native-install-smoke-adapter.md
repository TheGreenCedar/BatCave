...
The JavaScript suite creates process-local plans and #111 capabilities, rejects injected adapter arguments, copied descriptors, same-asset receipt substitution, and equivalent-plan replay, checks the two distinct macOS profiles, and confirms every source result remains skipped with null native/evidence receipts. The Rust probe copies fixture bytes into an immutable owned stream, rejects hostile tar entries and macOS filesystem collisions before materialization, rechecks every staged file, proves one-shot completion identity, and removes or explicitly retains its staging root. Existing validation and release workflows run both suites on Windows, Linux, and universal macOS.
