...
The deb profile rejects any pre-existing BatCave package state, verifies the expected package name, version, architecture, and installed executable ownership, and retains the installed package-owned file inventory. It installs and purges the private copy through fixed absolute `sudo systemd-run` transient services with control-group kill semantics, then requires the package record, GUI, CLI, and every observed package-owned file to be absent while current-user state remains. The AppImage profile launches the private AppImage through its own extract-and-run runtime, which avoids a hosted-runner FUSE dependency while retaining runtime `APPIMAGE` provenance, then removes the staged package. Both profiles require bounded output, timeout, direct-child exit, and process settlement before application removal or workspace cleanup.
