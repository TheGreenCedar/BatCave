...
The macOS profiles now have a reviewed source descriptor, but no platform process adapter is registered. Valid source-slice execution still returns `skipped` and emits no evidence. The descriptor binds verified asset identity and future tool/resource constraints; it does not retain the live capability or execute those tools. Native proof still requires a closed adapter to make the installer or stager consume the owned bytes while it performs trust, runtime, timeout-settlement, removal, and residue gates. The harness does not publish releases, sign packages, run installers, prove accessibility, prove Windows service or ETW behavior, prove updater expiry or A-to-B updates, or satisfy the stable-release gate.
