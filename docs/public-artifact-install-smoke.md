...
The built-in Linux deb/AppImage source descriptor is registered, and its fixed process-group settlement probes are covered by hosted validation. It has no package command or capability consumer, so Linux source execution still returns `skipped` and emits no evidence. Other platform adapters remain unavailable. Native proof still requires a closed adapter to make the installer or stager consume the owned bytes while it performs trust, runtime, timeout-settlement, removal, and residue gates. The harness does not publish releases, sign packages, run installers, prove accessibility, prove Windows service or ETW behavior, prove updater expiry or A-to-B updates, or satisfy the stable-release gate.
