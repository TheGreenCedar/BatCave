...
5. `downloadAndInstall()` looks up that in-memory resource, clones it for the operation, downloads the selected URL, verifies the payload bytes, and installs only after verification. It does not close the stored `Update` resource. BatCave does not explicitly call `Update.close()`. Replacing or clearing `pendingUpdate` therefore drops only the frontend reference; [Tauri's `Resource` contract keeps the Rust resource until explicit close or application exit](https://github.com/tauri-apps/tauri/blob/6f6ab1207bb3923c2721fbc67d2fdb1c8deb0c7a/packages/api/src/core.ts#L293-L334). Neither side writes this update selection to disk.
6. After a download or install failure, [`App.svelte`](../../src/BatCave.App/src/App.svelte) sets the update status to `error`. [`SettingsDrawer.svelte`](../../src/BatCave.App/src/lib/components/shell/SettingsDrawer.svelte) routes **Retry** to `onCheckForUpdates`, which clears `pendingUpdate` and fetches metadata again. If an update is still available, the next **Download and install** action downloads it from the new selection; Retry does not reuse the previous JavaScript `Update`. Today the abandoned Rust resource from the previous selection remains until explicit close or application exit.
...
| The user retries after payload verification or installation fails | `App.svelte` sets the status to `error`, and **Retry** invokes `onCheckForUpdates`. That path clears the frontend selection, fetches metadata again, and, if an update is still available, requires a new **Download and install** action that downloads from the new selection. It does not reuse the previous JavaScript `Update`. Until #47 adds explicit cleanup, the abandoned Rust resource remains in process until `Update.close()` or application exit; no update authorization survives process exit. |
...
Issue #47 should also close the current `Update` explicitly before clearing or replacing it on a new check and after a download/install failure. That close-on-replace/failure cleanup bounds abandoned Rust resources; it does not add persisted authorization, freshness, or anti-rollback state.

## Follow-up scope

Issue #47 can implement and test the corrected boundary without publishing a release. It should include explicit close-on-replace/failure handling for the frontend `Update` resource. A local fixture endpoint and disposable signed payloads can prove generic fail-closed behavior for unavailable, missing, and malformed metadata; equal/lower-version suppression; wrong-key and byte-tamper rejection; request failure; and offline monitoring. The fixture must exercise Tauri's native updater path rather than substitute a mock for the signature boundary.
