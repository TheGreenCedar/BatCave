...
A future closed macOS DMG adapter must rederive bundle identity, version, the supported Apple Silicon `arm64` architecture, signature integrity, Developer ID authority, notarization, and staple status from the copied app at the isolated consumed destination. A source-side observation, mounted-app observation, caller assertion, or evidence field cannot satisfy a destination gate.
...
- builds an inert Apple Silicon local app, ad-hoc signs it, places it in a local read-only DMG, and copies captured image bytes into an authority-owned root;
...
- rederives the bundle ID, version, required `arm64` architecture and absence of an Intel `x86_64` slice, code-signature integrity, Developer ID authority, notarization, and staple from that destination with fixed system tools;
...
1. independently verifies the immutable public release, full asset inventory, checksum, attestations, source identity, and selected Apple Silicon DMG bytes;
