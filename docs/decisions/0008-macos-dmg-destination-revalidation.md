...
This decision does not revise ADR 0006. `hdiutil` still cannot consume the Rust-owned descriptor, and a private filesystem path still has an unclosed same-user replace/open race. The local fixture therefore exercises the downstream mount/copy/revalidation lifecycle but always records `exact_transport_proven: false`. It cannot create a native execution receipt or evidence packet even if a synthetic gate observation is all true.
...
- rejects image substitution before attachment, deterministic substitution after preflight, invalid image mount, destination symlink substitution, bundle drift, executable trust drift, mount/copy timeout, and cleanup failure; and
- retains deliberate cleanup failure for a bounded retry and reports no path, command output, signature text, receipt, or evidence.
