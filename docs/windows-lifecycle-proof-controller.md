...
Two plan-bound fixtures are still required before the matrix can be closed:

- The pinned baseline and final packages both advertise `0.2.0-rc.2` and collector protocol `1`, so pairing their desktop and service binaries cannot produce a truthful incompatible-service state. The plan must bind a private service fixture with a deliberately incompatible release or protocol identity that still passes the fixed installed path and LocalSystem transport boundary.
- A successful production upgrade proves the commit path but cannot prove rollback. Rollback observation requires a separate plan-bound candidate or fixed fault fixture; an ad hoc corruption or caller-selected failure is not allowed.
