...
Any failed mutation captures both a pre-settlement and post-settlement snapshot, terminates and settles the owned Job, writes one nonce-bound failure response, and stops. It does not retry with a different command, controller, installer path, uninstaller form, service tool, or cleanup strategy.
