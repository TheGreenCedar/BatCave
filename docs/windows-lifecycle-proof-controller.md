...
The stacked desktop-evidence contract now makes each phase report a standard-token desktop identity, a bounded process tree with parent-process ancestry, canonical WebView Runtime identities, the typed visible collector state, privileged-source state, fallback/ETW truth, and the exact active or incompatible service identity needed by that phase. A passing fallback is therefore not an error-shaped result: it must prove that the intended service failure is visible while fresh standard monitoring continues with fallback ETW disabled. The second-instance phase is fixed to launch and settle both copies inside one owned phase; its result carries the attempted process identity and exit, the focused primary process identity, and the equal before/after service instance and collector generations rather than worker-authored booleans.

The same source slice defines a fail-closed sanitized path boundary and a deny-unknown export packet. Private packets retain exact paths, while export paths must map to one allowlisted logical root and one relative leaf. UNC paths, traversal, root-prefix collisions, paths outside the allowlist, drive-qualified and drive-relative values, alternate data streams, Windows device names, and the nonce-qualified evidence root are rejected. The typed export must bind the exact 28-file private success manifest, all six desktop phases, their sanitized machine/process observations, crash and rollback events, exact source/controller/plan identities, total final product absence, and preserved current-user data. It also requires a coherent service/process/ETW generation timeline, exact same-packet desktop-versus-machine runtime identity, production-shaped service instance IDs, and typed service/root ACL policy observations that do not drift across installed stages. The parent retains the six live desktop results and requires the exported desktop observations to match them exactly. The complete private-packet schema, parent-side redaction/projection comparison, and export writer are still part of the remaining lifecycle sequence; a separate `lifecycle_private_evidence_projection_not_reviewed` gate prevents controller execution until that independent byte-to-export binding exists.
...
Final success cannot be asserted with booleans. The worker must return the exact fixed 28-leaf private manifest with distinct size/SHA-256 receipts plus the fixed `windows-lifecycle-proof.sanitized.json` leaf; missing, additional, renamed, or duplicate receipts fail the result. After the worker Job settles, the parent opens and revalidates every named file under the nonce-bound protected root, validates the typed sanitized packet against those exact receipts, the preflight plan/controller identities, and the exact parent-observed desktop results, and retains all handles through the final response. The still-missing typed private-packet projection must make the parent derive and compare the sanitized semantics from those verified private bytes; until it does, the separate private-projection readiness gate remains false and success is unreachable. Failure results cannot carry success receipts.
