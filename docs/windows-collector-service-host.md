...
This host starts process-network ETW only after acquiring the protected root-bound owner guard, proving both the lease and exact session absent, and durably writing an `intent` lease. Consumer startup advances the lease to `active`; shutdown writes `stopping`, stops the collector engine, bounds consumer settlement, proves native session absence, and only then removes the exact lease. The service binary digest identifies its generation, the protected root's volume/file identity identifies the installation, and Windows supplies the boot identifier. Existing, corrupt, conflicting, or unqueryable lease/session state stays untouched and keeps the service unavailable; crashed-owner reclaim is intentionally deferred.
