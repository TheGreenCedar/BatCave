...
- npm now reports zero high and five moderate findings. A PostCSS-only override resolves Nano ID 5.1.16, tested against real synchronous and asynchronous transforms, parsing, generated IDs, errors, and source maps on the required Node 24. The remaining reports stem from PostCSS advisory GHSA-fxqj-rqcc-2cmp; its named 8.5.23 fix is unavailable from the registry. The audit remains failing and retains its JSON report. This override requires Node's synchronous ES-module loading through `require()`; disabling that feature is unsupported.
