...
Parent issue #115 remains open. The existing Rust-owned entry already re-establishes the public release and hands exact sealed bytes to the Linux handler. A later lane must make the fixed deb installer or AppImage stager consume those exact owned bytes and complete package trust, launch, release identity, settings restart, degradation, telemetry, removal, process cleanup, and user-state gates.
