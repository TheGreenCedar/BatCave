...
mod tests {
    ...
    #[test]
    #[ignore]
    fn live_collector_state_probe() {
        let process_degrades = |process: &ProcessSample| {
            process.quality.as_ref().is_none_or(|quality| {
                [
                    quality.cpu.as_ref(),
                    quality.memory.as_ref(),
        ...
        for tick in 0..60_u32 {
            let sample = collector.collect().expect("collect");
            let degrading: Vec<String> = sample
                .processes
                .iter()
                .filter(|process| process_degrades(process))
                .map(|process| format!("{}({})", process.name, process.pid))
                .collect();
            let system_degrading: Vec<&str> = sample
                .system
                .quality
                .as_ref()
                .map(|quality| {
                    [
                        ("cpu", quality.cpu.as_ref()),
                        ("kernel_cpu", quality.kernel_cpu.as_ref()),
                        ("logical_cpu", quality.logical_cpu.as_ref()),
                        ("memory", quality.memory.as_ref()),
                        ("swap", quality.swap.as_ref()),
                        ("disk", quality.disk.as_ref()),
                        ("network", quality.network.as_ref()),
                    ]
                    .into_iter()
                    .filter(|(_, q)| q.is_some_and(metric_degrades_collector))
                    .map(|(name, _)| name)
                    .collect()
                })
                .unwrap_or_default();
            eprintln!(
                "tick {tick}: state={:?} warnings={:?} degrading_processes={}/{} {:?} system_degrading={:?}",
