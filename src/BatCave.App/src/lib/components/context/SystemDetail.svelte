...
<section class="system-detail" aria-label="Resource detail view">
  ...
  {:else if detailMode === "memory"}
    ...
    <div class="detail-summary compact-summary" aria-label="Memory summary">
      <div><span>Used</span><strong>{memoryValue(snapshot.system.memory_used_bytes, formatBytes)}</strong></div>
      <div><span>Available</span><strong>{memoryValue(snapshot.system.memory_available_bytes, optionalBytes)}</strong></div>
    ...
    <div class="detail-chart-grid two-up compact-charts">
      <div class="detail-chart-card">
        <div><span>Memory load</span><strong>{memoryValue(memoryPercent, formatPercent)}</strong></div>
  ...
  {:else if detailMode === "disk"}
    ...
    <div class="detail-summary compact-summary" aria-label="Disk summary">
      <div><span>Read</span><strong>{diskValue(diskReadRate, formatRate)}</strong></div>
      <div><span>Write</span><strong>{diskValue(diskWriteRate, formatRate)}</strong></div>
      <div><span>Read total</span><strong>{diskValue(snapshot.system.disk_read_total_bytes, formatBytes)}</strong></div>
      <div><span>Write total</span><strong>{diskValue(snapshot.system.disk_write_total_bytes, formatBytes)}</strong></div>
    </div>
    <div class="detail-chart-grid two-up compact-charts">
      <div class="detail-chart-card">
        <div><span>Read throughput</span><strong>{diskValue(diskReadRate, formatRate)}</strong></div>
        <MiniChart values={history.diskRead} max={diskScaleMax} stroke={activeTheme.diskReadStroke} fill={activeTheme.diskReadFill} />
      </div>
      <div class="detail-chart-card">
        <div><span>Write throughput</span><strong>{diskValue(diskWriteRate, formatRate)}</strong></div>
  ...
  {:else}
    ...
    <div class="detail-summary compact-summary" aria-label="Network summary">
      <div><span>Download</span><strong>{networkValue(networkDownRate, formatRate)}</strong></div>
      <div><span>Upload</span><strong>{networkValue(networkUpRate, formatRate)}</strong></div>
      <div><span>Received</span><strong>{networkValue(snapshot.system.network_received_total_bytes, formatBytes)}</strong></div>
      <div><span>Sent</span><strong>{networkValue(snapshot.system.network_transmitted_total_bytes, formatBytes)}</strong></div>
    </div>
    <div class="detail-chart-grid two-up compact-charts">
      <div class="detail-chart-card">
        <div><span>Download rate</span><strong>{networkValue(networkDownRate, formatRate)}</strong></div>
        <MiniChart values={history.netRx} max={networkScaleMax} stroke={activeTheme.networkDownStroke} fill={activeTheme.networkDownFill} />
      </div>
      <div class="detail-chart-card">
        <div><span>Upload rate</span><strong>{networkValue(networkUpRate, formatRate)}</strong></div>
