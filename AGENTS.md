...
- `src/BatCave.App/`: production Rust + Tauri + Svelte desktop app, including frontend UI, Rust runtime store, native telemetry collectors, app icon, helper modes, benchmark CLI, and NSIS packaging config.
...
- Persistence, warm cache, helper snapshots, and logs are local-only under `%LOCALAPPDATA%\BatCaveMonitor`.
