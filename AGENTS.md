...
- `src/BatCave.App/`: production Rust + Tauri + Svelte desktop app, including frontend UI, Rust runtime store, native Windows/Linux/macOS telemetry collectors, app icon, helper modes, benchmark CLI, and platform packaging config.
...
- Persistence, warm cache, helper snapshots, and logs are local-only under `%LOCALAPPDATA%\BatCaveMonitor` on Windows, `$XDG_DATA_HOME/BatCaveMonitor` or `~/.local/share/BatCaveMonitor` on Linux, and `~/Library/Application Support/BatCaveMonitor` on macOS.
