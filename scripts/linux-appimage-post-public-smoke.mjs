...
const OUTPUT_FILE = path.join(OUTPUT_DIRECTORY, "linux-appimage-observation.json");
...
function parseSelectors(argv) {
  if (argv.length !== 2) {
    fail("usage: node scripts/linux-appimage-post-public-smoke.mjs <tag> <source-sha>");
  }
...
async function run(selectors) {
  if (process.platform !== "linux") fail("post-public AppImage smoke requires Linux");
  if (process.getuid?.() === 0) fail("post-public AppImage smoke must start as a standard user");
  if (process.arch !== "x64") fail("post-public AppImage smoke requires the amd64 release host");
  verifyWorkspaceReleaseVersion(selectors.tag);

  const workspace = fs.realpathSync(
    fs.mkdtempSync(path.join(os.tmpdir(), "batcave-linux-appimage-post-public-")),
  );
  ...
  try {
    ...
    const result = await linuxPersistenceCaptureInternals.captureVerifiedPublicAppImage(
...
export const linuxAppImagePostPublicSmokeInternals = {
  parseSelectors,
